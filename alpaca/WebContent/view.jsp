		...
		<table class="table table-dark table-hover text-success">
			...
			<tr>
				...
				if(name==null){
					%>
					파일이 없습니다.<br>
					<form action="fileUpdate.jsp?bnum=<%=num %>" method="post" enctype="multipart/form-data">
					...
					<button type="button" class="btn btn-info" onclick="fileDeleteFucntion()">업로드 삭제</button>
					<script>
					function fileDeleteFucntion(){
						if(confirm("삭제하시겠습니까?")){
							location.href="fileDelete.jsp?bnum=<%=num %>";
						}else{
							alert("취소되었습니다.");
						}
					}
